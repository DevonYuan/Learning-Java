...
public class UserBaseManager {
    ...
    // EFFECTS: Prints out the commands the user has available to them
    public static void printOptions() {
        System.out.println("1. Create a new account");
        System.out.println("2. Log in to an existing account");
        System.out.println("3. Reload the program from the previous state");
        System.out.println("Type \'Terminate\' to stop the program");
        System.out.print("Enter your command: ");
    }

    // EFFECTS: Saves the current state of the program, only if the user specifies
    // that they want to by interacting with the scanner.
    // MODIFIES: UsersData.json
    public void save() {
        System.out.println("Would you like to save the state of the program?");
        System.out.println("1. Yes");
        System.out.println("2. No");
        String command = scanner.nextLine();

        boolean gettingCommand = true;
    ...
    // EFFECTS: Saves the data to UsersData.json 
    public void saveData() {
        try {
            JsonWriter writer = new JsonWriter("./data/UsersData.json");
            writer.open();
    ...
    // EFFECTS: Loads the previous state of the program from the file.
    public void loadPreviousState() {
        System.out.println("\nLoading data from the file ...");
        try {
            this.userbase = loadFromFile();
        } catch (IOException e) {
            System.out.println("Coudln't read data from the file.");
        }
        System.out.println("Done!");
    }

    // EFFECTS: Gets the userbase with the state described in the file
    public UserBase loadFromFile() throws IOException {
        JsonReader reader = new JsonReader("./data/UsersData.json");
        UserBase previousState = reader.read();
    ...
    // EFFECTS: List all the solves done in a practice session
